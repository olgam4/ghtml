...
// build/dev/javascript/example_07_tailwind/components/alert_error.mjs
function render(message) {
  return div(toList([class$("rounded-md bg-red-50 p-4 mb-4")]), toList([
    div(toList([class$("flex")]), toList([
      div(toList([class$("ml-3")]), toList([
        p(toList([class$("text-sm font-medium text-red-800")]), toList([text2(message)]))
      ]))
    ]))
  ]));
}

// build/dev/javascript/example_07_tailwind/components/alert_success.mjs
function render2(message) {
  return div(toList([class$("rounded-md bg-green-50 p-4 mb-4")]), toList([
    div(toList([class$("flex")]), toList([
      div(toList([class$("ml-3")]), toList([
        p(toList([class$("text-sm font-medium text-green-800")]), toList([text2(message)]))
      ]))
    ]))
  ]));
}
...
// build/dev/javascript/example_07_tailwind/components/button_primary.mjs
function render3(label, on_click2) {
...
// build/dev/javascript/example_07_tailwind/components/button_secondary.mjs
function render4(label, on_click2) {
  return button(toList([
...
// build/dev/javascript/example_07_tailwind/components/card.mjs
function render5(title, body, image_url) {
  return div(toList([
    ...
    div(toList([class$("px-6 py-4")]), toList([
      h3(toList([class$("font-bold text-xl mb-2 text-gray-800")]), toList([text2(title)])),
      p(toList([class$("text-gray-600 text-base")]), toList([text2(body)]))
    ]))
  ]));
}

// build/dev/javascript/example_07_tailwind/types.mjs
class NavItem extends CustomType {
  constructor(label, href2) {
    super();
    this.label = label;
    this.href = href2;
  }
...
// build/dev/javascript/example_07_tailwind/components/navbar.mjs
function render6(brand, items) {
  return nav(toList([class$("bg-gray-800 shadow-lg")]), toList([
    div(toList([class$("max-w-7xl mx-auto px-4")]), toList([
      div(toList([class$("flex justify-between h-16")]), toList([
        div(toList([class$("flex items-center")]), toList([
          span(toList([class$("text-white font-bold text-xl")]), toList([text2(brand)]))
        ])),
        div(toList([class$("flex items-center space-x-4")]), toList([
          fragment2(index_map(items, (item, idx) => {
            return [
              to_string(idx),
              a(toList([
                href(item.href),
...
// build/dev/javascript/example_07_tailwind/app.mjs
var FILEPATH = "src/app.gleam";

class Model extends CustomType {
  constructor(button_clicks, show_success, show_error) {
    super();
    this.button_clicks = button_clicks;
    this.show_success = show_success;
    this.show_error = show_error;
  }
}

class IncrementClicks extends CustomType {
}

class ToggleSuccess extends CustomType {
}

class ToggleError extends CustomType {
}
function init(_) {
  return new Model(0, false, false);
}
function update2(model, msg) {
  if (msg instanceof IncrementClicks) {
    return new Model(model.button_clicks + 1, model.show_success, model.show_error);
  } else if (msg instanceof ToggleSuccess) {
...
function view(model) {
  return div(toList([class$("min-h-screen bg-gray-100")]), toList([
    ...
    main(toList([
      ...
      div(toList([class$("text-center mb-12")]), toList([
        h1(toList([
          class$("text-4xl font-bold text-gray-900 mb-4")
        ]), toList([text3("Tailwind CSS Example")])),
        p(toList([
          class$("text-xl text-gray-600 max-w-2xl mx-auto")
        ]), toList([
          text3("This example demonstrates how to use Tailwind CSS utility classes in Lustre templates. All styling is done with Tailwind utilities.")
        ]))
      ])),
      div(toList([class$("mb-8")]), toList([
        ...
        p(toList([class$("text-gray-600 mb-4")]), toList([
          text3("Tailwind makes it easy to create variant components. Each alert variant is a separate template with its own color scheme.")
        ])),
        (() => {
          let $ = model.show_success;
          if ($) {
            return render2("Operation completed successfully!");
          } else {
            return text3("");
          }
        })(),
        (() => {
          let $ = model.show_error;
          if ($) {
            return render("An error occurred. Please try again.");
          } else {
            return text3("");
          }
        })(),
        div(toList([class$("flex gap-2")]), toList([
          render3("Toggle Success", () => {
            return new ToggleSuccess;
          }),
          render4("Toggle Error", () => {
            return new ToggleError;
          })
        ]))
      ])),
      div(toList([class$("mb-8")]), toList([
        ...
        div(toList([class$("flex gap-4 flex-wrap")]), toList([
          render3("Primary Action", () => {
            return new IncrementClicks;
          }),
          render4("Secondary Action", () => {
            return new IncrementClicks;
          })
        ]))
      ])),
      div(toList([class$("mb-8")]), toList([
        h2(toList([
          class$("text-2xl font-semibold text-gray-800 mb-4")
        ]), toList([text3("Responsive Card Grid")])),
        p(toList([class$("text-gray-600 mb-4")]), toList([
          text3("Cards in a responsive grid that adapts from 1 column on mobile to 3 columns on large screens. Resize your browser to see it in action.")
        ])),
        div(toList([
          class$("grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6")
        ]), toList([
          render5("Utility-First CSS", "Tailwind provides low-level utility classes that let you build completely custom designs without leaving your HTML.", "https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=400&h=200&fit=crop"),
          render5("Responsive Design", "Use responsive modifiers like md: and lg: to build responsive interfaces. Mobile-first approach out of the box.", "https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?w=400&h=200&fit=crop"),
          render5("State Variants", "Add hover:, focus:, and active: prefixes to any utility for interactive states. No custom CSS needed.", "https://images.unsplash.com/photo-1551650975-87deedd944c3?w=400&h=200&fit=crop")
        ]))
      ])),
      div(toList([
        ...
        h3(toList([
          class$("text-lg font-medium text-yellow-800 mb-2")
        ]), toList([text3("Dynamic Classes Note")])),
        p(toList([class$("text-yellow-700")]), toList([
          text3("Tailwind Play CDN scans your HTML for class names at runtime. Dynamically constructed class names (like 'bg-' <> color) won't be detected. Create separate components for each variant instead.")
        ]))
      ]))
    ]))
  ]));
...
// build/dev/javascript/example_07_tailwind/example_07_tailwind.mjs
function main3() {
  return main2();
}

// .lustre/build/example_07_tailwind.mjs
main3();
