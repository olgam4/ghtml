...
// build/dev/javascript/example_03_events/components/counter.mjs
function render(count, on_increment, on_decrement) {
  return div(toList([class$("counter")]), toList([
    button(toList([class$("btn"), on_click(on_decrement())]), toList([text2("-")])),
    span(toList([class$("count")]), toList([text2(to_string(count))])),
    button(toList([class$("btn"), on_click(on_increment())]), toList([text2("+")]))
  ]));
}

// build/dev/javascript/example_03_events/components/form.mjs
function render2(value2, on_input2, on_click2, on_focus2, on_blur2) {
  return div(toList([class$("form")]), toList([
    input(toList([
      type_("text"),
      class$("input"),
      value(value2),
    ...
    button(toList([class$("btn"), on_click(on_click2())]), toList([text2("Submit")]))
  ]));
}

// build/dev/javascript/example_03_events/components/search_input.mjs
function render3(query, on_change) {
  return div(toList([class$("search")]), toList([
    input(toList([
      type_("search"),
      class$("input"),
      value(query),
...
// build/dev/javascript/example_03_events/app.mjs
var FILEPATH = "src/app.gleam";

class Model extends CustomType {
  constructor(count, query, form_value, is_focused) {
    super();
    this.count = count;
    this.query = query;
...
class Increment extends CustomType {
}

class Decrement extends CustomType {
}

class UpdateQuery extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class UpdateFormValue extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class ClickSubmit extends CustomType {
}

class FocusInput extends CustomType {
}

class BlurInput extends CustomType {
}
function init(_) {
  return new Model(0, "", "", false);
}
function update2(model, msg) {
  if (msg instanceof Increment) {
    return new Model(model.count + 1, model.query, model.form_value, model.is_focused);
  } else if (msg instanceof Decrement) {
    return new Model(model.count - 1, model.query, model.form_value, model.is_focused);
  } else if (msg instanceof UpdateQuery) {
    let q = msg[0];
    return new Model(model.count, q, model.form_value, model.is_focused);
  } else if (msg instanceof UpdateFormValue) {
    let v = msg[0];
    return new Model(model.count, model.query, v, model.is_focused);
  } else if (msg instanceof ClickSubmit) {
    return new Model(model.count, model.query, "", model.is_focused);
  } else if (msg instanceof FocusInput) {
    return new Model(model.count, model.query, model.form_value, true);
  } else {
    return new Model(model.count, model.query, model.form_value, false);
  }
}
function view(model) {
  return div(toList([]), toList([
    h1(toList([]), toList([text3("Events Example")])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Pattern 2: Function Call (@click={handler()})")])),
      p(toList([]), toList([
        text3("Use this pattern when you don't need event data. The handler is invoked directly.")
      ])),
      render(model.count, () => {
    ...
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([
        text3("Pattern 1: Function Reference (@input={handler})")
      ])),
      p(toList([]), toList([
        text3("Use this pattern when Lustre needs to extract and pass event data to your handler.")
      ])),
      render3(model.query, (var0) => {
        return new UpdateQuery(var0);
      }),
      p(toList([class$("result")]), toList([text3("You typed: " + model.query)]))
    ])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Multiple Event Types")])),
      p(toList([]), toList([
        text3("Combining both patterns: @input uses function reference, @click/@focus/@blur use function calls.")
      ])),
      render2(model.form_value, (var0) => {
        return new UpdateFormValue(var0);
      }, () => {
        return new ClickSubmit;
      }, () => {
        return new FocusInput;
      }, () => {
        return new BlurInput;
      }),
      p(toList([class$("status")]), toList([
        text3((() => {
...
// build/dev/javascript/example_03_events/example_03_events.mjs
function main2() {
  return main();
}

// .lustre/build/example_03_events.mjs
main2();
