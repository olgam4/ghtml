...
var createComment = (data2) => document().createComment(data2);
...
function new$5(options) {
  ...
  return fold2(options, init, (config, option2) => {
...
// build/dev/javascript/example_04_control_flow/components/if_without_else.mjs
function render(show_warning, message) {
  return div(toList([class$("notification")]), toList([
    (() => {
      if (show_warning) {
        return p(toList([class$("warning")]), toList([text2(message)]));
      } else {
        return none2();
      }
    })()
...
// build/dev/javascript/example_04_control_flow/types.mjs
...
class Online extends CustomType {
}
class Away extends CustomType {
  constructor(reason) {
    super();
    this.reason = reason;
  }
}
class Offline extends CustomType {
}
...
class Todo extends CustomType {
  constructor(id, text4, completed, priority) {
    super();
    this.id = id;
    this.text = text4;
...
// build/dev/javascript/example_04_control_flow/components/item_list.mjs
function render2(items) {
  return ul(toList([class$("item-list")]), toList([
    fragment2(index_map(items, (item, index4) => {
      return [
        to_string(index4),
        li(toList([class$("item")]), toList([
          span(toList([class$("index")]), toList([text2(to_string(index4)), text2(".")])),
          span(toList([class$("name")]), toList([text2(item.name)]))
        ]))
      ];
    }))
  ]));
...
// build/dev/javascript/example_04_control_flow/components/status_display.mjs
function render3(status) {
  return div(toList([class$("status-display")]), toList([
    (() => {
      if (status instanceof Online) {
        return span(toList([class$("online")]), toList([text2("Online")]));
      } else if (status instanceof Away) {
        let reason = status.reason;
        return span(toList([class$("away")]), toList([text2("Away: "), text2(reason)]));
      } else {
        return span(toList([class$("offline")]), toList([text2("Offline")]));
      }
    })()
  ]));
}

// build/dev/javascript/example_04_control_flow/components/todo_item.mjs
function render4(t, on_toggle, on_delete) {
  return div(toList([class$("todo-item")]), toList([
    ...
    span(toList([class$("text")]), toList([
      (() => {
        let $ = t.completed;
        if ($) {
          return s(toList([]), toList([text2(t.text)]));
        } else {
          return text2(t.text);
        }
      })()
    ...
    (() => {
      let $ = t.priority;
      if ($ instanceof High) {
        return span(toList([class$("priority high")]), toList([text2("!")]));
      } else if ($ instanceof Medium) {
        return span(toList([class$("priority medium")]), toList([text2("-")]));
      } else {
        return span(toList([class$("priority low")]), toList([text2(".")]));
      }
    })(),
    button(toList([class$("delete"), on_click(on_delete())]), toList([text2("x")]))
  ]));
}

// build/dev/javascript/example_04_control_flow/components/user_badge.mjs
function render5(is_admin) {
  return span(toList([class$("badge")]), toList([
    (() => {
      if (is_admin) {
        return span(toList([class$("admin")]), toList([text2("Admin")]));
      } else {
        return span(toList([class$("member")]), toList([text2("Member")]));
      }
    })()
  ]));
}

// build/dev/javascript/example_04_control_flow/app.mjs
var FILEPATH = "src/app.gleam";

class Model extends CustomType {
  constructor(is_admin, show_warning, items, status, todos, new_todo_text, new_todo_priority) {
    super();
    ...
    this.status = status;
    this.todos = todos;
    this.new_todo_text = new_todo_text;
    this.new_todo_priority = new_todo_priority;
  }
}

class ToggleAdmin extends CustomType {
}

class ToggleWarning extends CustomType {
}

class SetStatus extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class ToggleTodo extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class DeleteTodo extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class UpdateNewTodoText extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class SetNewTodoPriority extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class AddTodo extends CustomType {
}
function init(_) {
  return new Model(true, true, toList([
    new Item("1", "Learn Gleam"),
    new Item("2", "Build with Lustre"),
    new Item("3", "Master templates")
  ]), new Online, toList([
    new Todo("1", "Read the docs", true, new Low),
    new Todo("2", "Try the examples", false, new Medium),
    new Todo("3", "Build something", false, new High)
  ]), "", new Medium);
}
function update2(model, msg) {
  if (msg instanceof ToggleAdmin) {
    return new Model(!model.is_admin, model.show_warning, model.items, model.status, model.todos, model.new_todo_text, model.new_todo_priority);
  } else if (msg instanceof ToggleWarning) {
    return new Model(model.is_admin, !model.show_warning, model.items, model.status, model.todos, model.new_todo_text, model.new_todo_priority);
  } else if (msg instanceof SetStatus) {
    let status = msg[0];
    return new Model(model.is_admin, model.show_warning, model.items, status, model.todos, model.new_todo_text, model.new_todo_priority);
  } else if (msg instanceof ToggleTodo) {
    let id = msg[0];
    return new Model(model.is_admin, model.show_warning, model.items, model.status, map2(model.todos, (t) => {
      let $ = t.id === id;
      if ($) {
        return new Todo(t.id, t.text, !t.completed, t.priority);
      } else {
        return t;
      }
    }), model.new_todo_text, model.new_todo_priority);
  } else if (msg instanceof DeleteTodo) {
    let id = msg[0];
    return new Model(model.is_admin, model.show_warning, model.items, model.status, filter(model.todos, (t) => {
      return t.id !== id;
    }), model.new_todo_text, model.new_todo_priority);
  } else if (msg instanceof UpdateNewTodoText) {
    let text4 = msg[0];
    return new Model(model.is_admin, model.show_warning, model.items, model.status, model.todos, text4, model.new_todo_priority);
  } else if (msg instanceof SetNewTodoPriority) {
    let priority = msg[0];
    return new Model(model.is_admin, model.show_warning, model.items, model.status, model.todos, model.new_todo_text, priority);
  } else {
    let id = to_string(length(model.todos) + 1);
    let new_todo = new Todo(id, model.new_todo_text, false, model.new_todo_priority);
    return new Model(model.is_admin, model.show_warning, model.items, model.status, append(model.todos, toList([new_todo])), "", model.new_todo_priority);
  }
}
function parse_priority(s2) {
  if (s2 === "high") {
    return new SetNewTodoPriority(new High);
  } else if (s2 === "low") {
    return new SetNewTodoPriority(new Low);
  } else {
    return new SetNewTodoPriority(new Medium);
  }
}
function view(model) {
  return div(toList([]), toList([
    h1(toList([]), toList([text3("Control Flow Example")])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("{#if}...{:else}...{/if}")])),
      p(toList([]), toList([
        text3("Conditional rendering based on a boolean expression. The else branch is optional.")
      ])),
      div(toList([class$("demo")]), toList([
        render5(model.is_admin),
        button(toList([
          class$("btn"),
          on_click(new ToggleAdmin)
        ]), toList([text3("Toggle Admin")]))
      ]))
    ])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("{#if}...{/if} (without else)")])),
      p(toList([]), toList([
        text3("When the condition is false and there's no else branch, nothing is rendered.")
      ])),
      div(toList([class$("demo")]), toList([
        render(model.show_warning, "This is a warning message!"),
        button(toList([
          class$("btn"),
          on_click(new ToggleWarning)
        ]), toList([text3("Toggle Warning")]))
      ]))
    ])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("{#each collection as item, index}...{/each}")])),
      p(toList([]), toList([
        text3("Iterate over a list. The index is optional. Uses keyed() for Lustre performance.")
      ])),
      div(toList([class$("demo")]), toList([render2(model.items)]))
    ])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("{#case expr}{:Pattern}...{/case}")])),
      p(toList([]), toList([
        text3("Pattern matching on custom types. Supports bindings in patterns.")
      ])),
      div(toList([class$("demo")]), toList([
        render3(model.status),
        div(toList([class$("controls")]), toList([
          button(toList([
            class$("btn"),
            on_click(new SetStatus(new Online))
          ]), toList([text3("Online")])),
          button(toList([
            class$("btn"),
            on_click(new SetStatus(new Away("lunch break")))
          ]), toList([text3("Away")])),
          button(toList([
            class$("btn"),
            on_click(new SetStatus(new Offline))
          ]), toList([text3("Offline")]))
        ]))
      ]))
    ])),
    div(toList([class$("section")]), toList([
      h2(toList([]), toList([text3("Combined Example: Todo List")])),
      p(toList([]), toList([
        text3("This todo list uses all three control flow constructs together.")
      ])),
      div(toList([class$("todo-list")]), toList([
        div(toList([class$("add-todo")]), toList([
          input(toList([
            class$("input"),
            placeholder("New todo..."),
            value(model.new_todo_text),
            on_input((var0) => {
              return new UpdateNewTodoText(var0);
            })
          ])),
          select(toList([
            on_input((s2) => {
              return parse_priority(s2);
            })
          ]), toList([
            option(toList([value("low")]), "Low"),
            option(toList([
              value("medium"),
              selected(true)
            ]), "Medium"),
            option(toList([value("high")]), "High")
          ])),
          button(toList([
            class$("btn"),
            on_click(new AddTodo)
          ]), toList([text3("Add")]))
        ])),
        div(toList([class$("items")]), map2(model.todos, (t) => {
          return render4(t, () => {
            return new ToggleTodo(t.id);
          }, () => {
            return new DeleteTodo(t.id);
          });
        }))
      ]))
    ]))
...
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError("let_assert", FILEPATH, "app", 17, "main", "Pattern match failed, no pattern matched the value.", { value: $, start: 438, end: 487, pattern_start: 449, pattern_end: 454 });
  }
  return;
}

// build/dev/javascript/example_04_control_flow/example_04_control_flow.mjs
function main2() {
  return main();
}

// .lustre/build/example_04_control_flow.mjs
main2();
